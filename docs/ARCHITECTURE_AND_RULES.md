...
- **خطة الجدولة والمراحل لإزالة المتبقي (Phased Removal Roadmap):**
  ...
  - **سجل مراحل التنفيذ المحققة (Progress Record):**
    ...
    * الخطوة الختامية (Final Quality & Code Hygiene): استحداث فئات الـ Utilities الذرية المعيارية، تنظيف الـ Inline Styles المباشرة، تحسين جودة وتناسق الـ Textareas، والقضاء التام على آخر قوائم منسدلة بشكل المتصفح القديم (v1.4.26).
...
## 16. معايير نظام التصميم الموحد للموبايل (Mobile Design System & UI Tokens Standard)
تلتزم جميع الواجهات المطورة لشاشات الهواتف المحمولة بالمحددات القياسية التالية:
1. **قواعد الأبعاد والأزرار (Geometry & Button Tokens):**
   - أزرار الإجراءات السريعة والكبسولات المصغرة: الارتفاع القياسي `30px`، ونصف قطر الحواف `8px`، وحجم خط `0.76rem`.
   - أزرار الاختيار والمدخلات القياسية: الارتفاع `38px - 42px`، ونصف قطر الحواف `10px`.
   - زر الحفظ والإرسال الرئيسي (Primary Action CTA): الارتفاع `44px`، ونصف قطر الحواف `12px`، وخط عريض واضح `0.95rem`.
   - الكروت والحاويات: نصف قطر الحواف `14px - 18px`، مع استخدام هوامش داخلية متزنة (`padding: 12px 14px`).
2. **قاعدة المسار الطبيعي ومنع التراكب (Single Document Flow & Zero Floating Overlaps):**
   - يُحظر تماماً تثبيت أزرار عائمة عشوائية (`position: fixed`) في أسفل شاشات الموبايل فوق شريط التنقل السفلي (`bottom-nav`)، لتفادي حجب السجلات اليومية أو التداخل مع التنقل.
   - يُحظر ظهور الصناديق المتداخلة المزدوجة (`Card-in-Card`) على الموبايل؛ فالأقسام التي تعرض كروت موبايل (`.mobile-only-cards-container`) يجب تصفير الحاوية الخارجية فيها تلقائياً (`background: transparent; border: none; padding: 0`).
3. **نظام الهوية البصرية للجنس الطبي (Clinical Gender Theming System):**
   - تمييز بصري فوري ومتناسق بين الذكور والإناث عبر كامل المنظومة:
     * **المرضى الذكور:** شريط علوي أزرق نيلي (`#0284c7`)، أيقونة مريض رجل (`fa-solid fa-person`)، شارة سن وجنس زرقاء (`badge-gender-male`)، وتوهج ليلي أزرق داكن.
     * **المريضات الإناث:** شريط علوي وردي/روز (`#ec4899`)، أيقونة مريضة سيدة (`fa-solid fa-person-dress`)، شارة سن وجنس وردية (`badge-gender-female`)، وتوهج ليلي بنفسجي داكن.
4. **مبدأ التسجيل الخاطف والمدمج (Fast-Track Check-in):**
   - دمج الحقول المرتبطة في صف واحد مدمج (مثل السن والنوع 50% / 50%).
   - استبدال القوائم المنسدلة التقليدية بأزرار كبسولات التبديل بنقرة واحدة (Segmented Controls) للبيانات الثنائية (ذكر/أنثى، نقدي/تأمين).
   - توفير كبسولات سريعة (Quick Chips) لأكثر الشركات استخداماً بدلاً من فتح نوافذ منبثقة متكررة.

---

## 17. معايير الاختبارات الآلية للحسابات الحساسة (Automated Financial & Clinical Tests Standard)
يُشترط نجاح الاختبارات الآلية بنسبة 100% عبر أمر `npm test` قبل اعتماد أي تحديث إنتاجي، وتغطي الاختبارات المحاور الحرجة التالية:
1. **معادلة رصيد وحدات التأمين المعتمدة:**
   - التحقق من تطبيق المعادلة الحسابية المعتمدة: `إجمالي الجلسات المصرح بها = عدد الزيارات المعتمدة بالجواب × عدد الأعضاء المصرح بها بالزيارة`.
2. **محددات تقدم دورة التأمين (Cycle Limits):**
   - كشف اقتراب انتهاء الموافقة: `isNearLimit = currentCount >= approvedTotal - 2`.
   - كشف اكتمال موافقة التأمين بالكامل: `isCompleted = currentCount >= approvedTotal`.
3. **المطابقة المالية للخزينة (Daily Cash Reconciliation):**
   - التحقق من معادلة صافي النقدية بالدرج: `صافي النقدية بالدرج = إجمالي المقبوضات النقدية - إجمالي المصروفات اليومية`.
4. **تجميع إحصائيات الأطباء والمطالبات:**
   - فحص صحة احتساب وتوزيع حالات النقدي والتأمين ووحدات العمل لكل طبيب معالج، ودقة جمع إجمالي المطالبات التأمينية في السجلات.
